...
const CropHealthReport: React.FC = () => {
  ...
  const { farm, report, shareUrl, generatedImages, isWebEnhanced } = (location.state as any) || {};

  // Function to enhance with web data
  const enhanceWithWebData = async () => {
    if (!farm) return;
    ...
    try {
      const response = await fetch('/api/firecrawl-analysis', {
        ...
        body: JSON.stringify({
          ...
          farmerId: farm.id,
          analysisType: 'quick-search' // Default to quick search for reports
        })
      });
      ...
      if (result) {
        setFirecrawlData(result.data);
        setWebEnhanced(true);
      }
    } catch (error) {
      console.error('Error enhancing with web data:', error);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-stone-50 pb-12 print:bg-white print:pb-0">
      ...
      <div className="fixed bottom-6 left-6 z-50 print:hidden">
        <button
          onClick={enhanceWithWebData}
          disabled={isEnhancing}
          className="bg-emerald-600 text-white px-6 py-3 rounded-full shadow-xl font-bold text-sm border border-emerald-700 hover:bg-emerald-700 flex items-center gap-2 transition-transform hover:-translate-y-1 disabled:opacity-50"
        >
          {isEnhancing ? (
            <>
      ...
      {(webEnhanced || (report as any)?.firecrawlInsights) && (
        <div className="max-w-6xl mx-auto px-8 pb-12">
          <div className="bg-white rounded-3xl shadow-xl border border-stone-200 p-8">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-black text-stone-900">Enhanced Insights from Web Data</h3>
            ...
            <div className="space-y-6">
              {(firecrawlData?.firecrawlInsights || (report as any)?.firecrawlInsights) ? (
                <div>
                  <h4 className="font-bold text-lg text-stone-800 mb-3">Latest Information</h4>
                  <p className="text-stone-700 leading-relaxed">
                    {(() => {
                      const data = firecrawlData?.firecrawlInsights || (report as any)?.firecrawlInsights;
                ...
                <p className="text-stone-500 italic">No additional web insights available.</p>
              )}

              {(firecrawlData?.combinedScore || report?.healthScore) && (
                <div className="pt-4 border-t border-stone-100">
                  <p className="text-lg font-bold text-stone-800">
                    Combined Health Score: <span className="text-emerald-600">{firecrawlData?.combinedScore || report?.healthScore}/100</span>
                  </p>
                </div>
              )}
            </div>
